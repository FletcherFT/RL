...
Original Author: Patrick Coady (pat-coady.github.io)
Modified by Fletcher Thompson
"""
import numpy as np
...
class Policy(object):
    ...
    def __init__(self, g, obs_dim, act_dim, kl_targ, hid1_mult, policy_logvar):
        """
        Args:
    ...
    def _policy_nn(self):
        ...
        out = tf.layers.dense(self.obs_ph, hid1_size, tf.tanh,
                              kernel_initializer=tf.random_normal_initializer(
                                  stddev=np.sqrt(1 / self.obs_dim)), name="pol_h1")
        out = tf.layers.dense(out, hid2_size, tf.tanh,
                              kernel_initializer=tf.random_normal_initializer(
                                  stddev=np.sqrt(1 / hid1_size)), name="pol_h2")
        out = tf.layers.dense(out, hid3_size, tf.tanh,
                              kernel_initializer=tf.random_normal_initializer(
                                  stddev=np.sqrt(1 / hid2_size)), name="pol_h3")
        self.means = tf.layers.dense(out, self.act_dim,
                                     kernel_initializer=tf.random_normal_initializer(
                                         stddev=np.sqrt(1 / hid3_size)), name="pol_output")
        # logvar_speed is used to 'fool' gradient descent into making faster updates
        # to log-variances. heuristic sets logvar_speed based on network size.
        logvar_speed = (10 * hid3_size) // 48
        with tf.name_scope("log-variances") as scope:
    ...
    def _logprob(self):
        ...
        """
        with tf.name_scope("log-likelihood") as scope:
    ...
    def _kl_entropy(self):
        ...
        """
        with tf.name_scope("policy_loss/KL_divergence") as scope:
    ...
    def _sample(self):
        ...
        with tf.name_scope("action_sampling") as scope:
    ...
    def _loss_train_op(self):
        """
        Three loss terms:
            1) standard policy gradient (A*logpi//logpiold)
            2) D_KL(pi_old || pi_new)
            3) Hinge loss on [D_KL - 2*kl_targ]^2, if kl < 2*kltarg; set to 0.0

        See: https://arxiv.org/pdf/1707.02286.pdf
        """
        with tf.name_scope("policy_loss") as scope:
            loss1 = -tf.reduce_mean(self.advantages_ph *
                                    tf.exp(self.logp - self.logp_old)) #regular policy gradient loss term
            loss2 = tf.reduce_mean(self.beta_ph * self.kl) #KL divergence loss (analogous to P control)
            loss3 = self.eta_ph * tf.square(tf.maximum(0.0, self.kl - 2.0 * self.kl_targ)) #on-off bang-bang controller
            self.loss = loss1 + loss2 + loss3
        with tf.name_scope("policy_optimizer") as scope:
            optimizer = tf.train.AdamOptimizer(self.lr_ph)
            self.train_op = optimizer.minimize(self.loss,name='adam')
    ...
    def update(self, sess, observes, actions, advantages, logger):
        ...
        for e in range(self.epochs):
            ...
            if kl > self.kl_targ * 4:  # early stopping if kl divergence is way too large
                break
        # The following is a servoing method to dynamically set Beta, so that D_KL target is reached.
        if kl > self.kl_targ * 2:  # servo beta to reach D_KL target
            self.beta = np.minimum(35, 1.5 * self.beta)  # clip beta to be no larger than 35 (chosen heuristic)
            if self.beta > 30 and self.lr_multiplier > 0.1: # if beta is high and learning rate high: shrink the learning rate multiplier
                self.lr_multiplier /= 1.5
        elif kl < self.kl_targ / 2: # if the kl divergence is less than half of the target:
            self.beta = np.maximum(1.0 / 35, self.beta / 1.5)  # clip beta to 1/35 minimum (another heuristic)
            if self.beta < (1.0 / 30) and self.lr_multiplier < 10: # if beta is small and learning rate multipler is low: grow the learning rate multiplier
                self.lr_multiplier *= 1.5
