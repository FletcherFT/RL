...
class NNValueFunction(object):
    ...
    def __init__(self, g, obs_dim, hid1_mult):
        """
        Args:
        ...
        with self.g.as_default():
            ...
            out = tf.layers.dense(self.obs_ph, hid1_size, tf.tanh,
                                  kernel_initializer=tf.random_normal_initializer(
                                      stddev=np.sqrt(1 / self.obs_dim)), name="val_h1")
            out = tf.layers.dense(out, hid2_size, tf.tanh,
                                  kernel_initializer=tf.random_normal_initializer(
                                      stddev=np.sqrt(1 / hid1_size)), name="val_h2")
            out = tf.layers.dense(out, hid3_size, tf.tanh,
                                  kernel_initializer=tf.random_normal_initializer(
                                      stddev=np.sqrt(1 / hid2_size)), name="val_h3")
            out = tf.layers.dense(out, 1,
                                  kernel_initializer=tf.random_normal_initializer(
                                      stddev=np.sqrt(1 / hid3_size)), name='val_output')
            self.out = tf.squeeze(out)
            with tf.name_scope('value_loss') as scope:
                self.loss = tf.reduce_mean(tf.square(self.out - self.val_ph))  # squared loss
            with tf.name_scope('value_optimizer') as scope:
