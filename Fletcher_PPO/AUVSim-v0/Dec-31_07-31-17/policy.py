...
class Policy(object):
    ...
    def update(self, sess, observes, actions, advantages, logger):
        ...
        if kl > self.kl_targ * 2:  # servo beta to reach D_KL target
            ...
            self.beta = np.maximum(1 / 35, self.beta / 1.5)  # clip beta to 1/35 minimum (another heuristic)
            if self.beta < (1 / 30) and self.lr_multiplier < 10: # if beta is small and learning rate multipler is low: grow the learning rate multiplier
                self.lr_multiplier *= 1.5
