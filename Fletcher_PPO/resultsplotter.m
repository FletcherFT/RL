envs = dir('*');
isub = [envs(:).isdir]&~ismember({envs(:).name},{'.','..'});
envs = envs(isub);

for ienvs =1:length(envs)

    logs = dir(fullfile(envs(ienvs).name,'*'));
    isub = [logs(:).isdir]&~ismember({logs(:).name},{'.','..'});
    logs = logs(isub);

    for i=1:length(logs)
        csvPath = fullfile(envs(ienvs).name,logs(i).name,'log.csv');
        ...
        h = findobj('Type','Figure','Name',fullfile(envs(ienvs).name,logs(i).name));
        if isempty(h) 
            F = figure('Name',fullfile(envs(ienvs).name,logs(i).name));
        else
            figure(h);
        end
        ix = strcmp(headers,'_Episode');
        iy = strcmp(headers,'_MeanReward');
    ...
    if strcmp(envs(ienvs).name,'AUVSim-v0')
        %% PLOT 2 VS 3 VS 6 DOF rewards
        h = findobj('Type','Figure','Name',fullfile(envs(ienvs).name,'2v3v6'));
        if isempty(h)
            F = figure('Name',fullfile(envs(ienvs).name,'2v3v6'));
        else
            figure(h);
        end
        idx = [8,7,5];
        ...
        h = findobj('Type','Figure','Name',fullfile(envs(ienvs).name, '200v500v1000'));
        if isempty(h)
            F = figure('Name',[envs(ienvs).name, ' 200v500v1000']);
        else
            figure(h);
        end
        idx = [10,8,4];
